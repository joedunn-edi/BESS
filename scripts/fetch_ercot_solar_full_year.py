...
day, 365 total, paced the same way as the RTM full-year backfill.
...
while d <= END_DATE:
    try:
        frames.append(fetch_with_retry(fetch_solar_generation, d, token=token, session=requests))
        print(f"  fetched {d}")
